...
export const retrievePublicKey = async () => {
  ...
  if (accessObj.error) {
    ...
    const myAddress = accessObj.address;

    shortenPublicKey(myAddress);
...
export function shortenPublicKey(myAddress) {
  if (myAddress.length >= 8) {
    const firstFourLetters = myAddress.slice(0, 4);
    const lastFourLetters = myAddress.slice(-4);
